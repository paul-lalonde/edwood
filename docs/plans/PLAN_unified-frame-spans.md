...
| # | Design | Tests | Iterate | Commit |
|---|---|---|---|---|
| B2.3.R1 | [ ] frame-layout-design §2.2 (lineSummary + FirstRune) + §3 + §3.3 (eager split + coalesce) | [ ] After `relayoutFrom`, `f.lines` non-empty iff `f.box` non-empty; `lines[i].{TopY, LineH, LineA, FirstRune}` match the corresponding `box[FirstBox]` rune-offset; line count matches visual-line count for plain text and for `test-md-layout`; I-LAYOUT-2, I-LAYOUT-3, I-LAYOUT-6 fixtures pass; split-then-coalesce round-trip (load long word, grow rect, relayout → single box); coalesce does not cross a hard newline or a wrap boundary | [ ] Add `lineSummary{FirstBox, FirstRune, TopY, LineH, LineA}` and `frameimpl.lines []lineSummary`; populate during phase B (per-line append); move long-word `splitbox` inline into phase A (§3.3 case 3) with multi-split propagation; add eager-coalesce pre-wrap check (§3.3) for same-style adjacent same-line boxes; no external consumer migration yet | [ ] `frame: per-line summary table + eager split/coalesce in relayoutFrom (B2.3 R1)` |
