...
| # | Design | Tests | Iterate | Commit | Notes |
|---|---|---|---|---|---|
| A2.1 | [ ] §5.1, §5.4 `InsertWithStyle` | [ ] nil-styles ≡ upstream `Insert`; all-default `StyleRun` slice ≡ fast path; multi-run color apply; mismatched Lens panic | [ ] Add `InsertWithStyle` on `Frame` and `SelectScrollUpdater`; per-rune Fg/Bg storage | [ ] `frame: add InsertWithStyle (color-only)` | Signature final from A2.1 forward; implementation grows. |
