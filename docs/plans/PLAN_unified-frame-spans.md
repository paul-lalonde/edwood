...
| B2.3.R3 | [x] frame-layout-design §4.2 + §4.4 | [x] 10 numbered requirements (R3.1–R3.10) in `frame/charofpt_line_table_test.go`: click-on-first / nth / left-of / above / below content; empty frame; plain-text parity (grid sweep); scaled-heading correctness; 50-line binary-search bound. | [x] Rewrote `charOfPtReader` in `frame/ptofchar.go` to binary-search `f.lines` by `TopY` (lines are Y-sorted by I-LAYOUT-3), then walk only that line's boxes via `FirstBox`..next-line's-FirstBox. End-of-content (pt.Y past last line bottom) derives the rune offset from the box list rather than `f.nchars`, so inline-constructed test fixtures work without manually setting `nchars`. Legacy `charofptimpl` retained for the `unlockedproxy` path (deleted in R11). | [x] (pending) `frame: Charofpt routes through line-summary table (B2.3 R3)` |
| B2.3.R4 | [x] frame-layout-design §6.4 — scoped: see Notes. | [x] 7 numbered requirements (R4.1–R4.5 + 2 extensions) in `frame/draw_accumulator_test.go` covering pt1 / tab tabstop alignment / overflow truncation / scroll-cycle integration. Existing TestBxscan / TestInsert / TestSetStyleRange suites still green. | [x] Added tab newwid call in `relayoutFrom`'s phase A (parent-frame path: SetStyleRange / Delete / resize). Stripped `_draw` of the redundant `canfit + splitbox` calls (R1's eager-split covers them). `_draw`'s pt-accumulator + truncation + tab-newwid retained for the bxscan staging-frame path — `_draw` is the only walker that knows pt0 (the insertion point in parent coords) needed for tab Wid recompute and pt1 simulation. Added `nextPositionAfterLast` and `truncateOffscreen` helpers (line-table-driven; ready for R6/R7 consumers but not yet wired). Stage 4: `TestSetStyleRange_PreservesTabWidth` updated to assert tabstop-aligned tabs (recomputed) rather than "preserved" — the old behavior left tabs misaligned when leading content widths changed. | [x] (pending) `frame: relayoutFrom recomputes tabs; _draw stripped of redundant logic (B2.3 R4)` |
| B2.3.R5 | [x] frame-layout-design §3.5 (snapshotLines + diffLines + paintOp). Design refined mid-row: matching is by content **digest** (FNV-1a hash over Ptr + Style.Kind + Bc + Nrune), not FirstRune. FirstRune isn't stable across rune-shifting mutations (insert at position 0 shifts every subsequent line's coord). | [x] 9 numbered requirements (R5.1–R5.9) in `frame/diff_lines_test.go`: snapshotLines one-entry-per-line / digest-stable / digest-sensitive; diffLines no-change / shifted-run / dirty-line / all-dirty / mixed-runs / screen-order. | [x] Added `lineSnap{FirstRune, TopY, LineH, Digest}`, `paintOp{Kind, Src, Dst}`, `paintOpKind` (OpPaint, OpBlit) in `frame/diff_lines.go`. `(*frameimpl).snapshotLines()`, `(*frameimpl).lineDigest(i)`, `(*frameimpl).diffLines(snap)` with forward-greedy digest matching and adjacent-same-ΔY run compression. No mutator wired yet — R6 is first consumer. | [x] (pending) `frame: snapshotLines + diffLines helpers (B2.3 R5)` |
| B2.3.R6 | [x] frame-layout-design §6.2 | [x] 5 new tests in `frame/delete_diff_test.go` (R6.1–R6.6). All existing TestDelete* sub-tests green after Stage-4 updates (see Iterate). `./regression.sh` green. | [x] Rewrote `deleteimpl` per §6.2: pre-mutation `snapshotLines` + bottom-Y capture; `findbox` splits at p0/p1 boundaries; `closebox` splices out; `relayoutFrom(0)` (eager-coalesce re-merges any boundary-split fragments); `diffLines(snap)` + `issuePaintOps` issues blits/paints; vacated bottom region cleared. Added `(*frameimpl).issuePaintOps` and `linesInDstYRange` in `frame/diff_lines.go`. Removed the legacy per-box `cklinewrap0`/`cklinewrap`/`canfit`/`splitbox`/`advance` inner loop, the trailing `if pt1.Y != pt0.Y` blit-the-remainder step, and the now-redundant `f.clean(...)` call (R1's eager-coalesce in relayoutFrom subsumes it). Stage-4: also fixed an R2 implementation accident — the `lastlinefull`-setting defer was registered AFTER an early-return for `nb0 == len(f.box)`, so Delete-to-empty didn't reset the field. Moved the defer up. Stage-4 wrong-test updates (with user approval): TestR7_Delete_BlitHeight_PlainLines/ScaledLine updated for R5's run-compressed multi-line blits; 4 TestDelete sub-cases updated for line-rect paint ops; 4 SVG baselines regenerated. | [x] (pending) `frame: deleteimpl uses snapshot + diff (B2.3 R6)` |
| B2.3.R7 | [x] frame-layout-design §6.1; mid-row design refinement to §3.5 diff (off-screen Src must paint, not blit) — see Iterate | [x] 6 new tests in `frame/insert_diff_test.go` (R7.1–R7.6). All TestInsert / TestInsertAligned sub-tests updated for the new line-batch op sequence; 9 SVG baselines regenerated. Manual smoke test confirmed the legacy "mid-screen content after scroll" + "missing heading" + "duplicate stripe after scroll" bugs all fixed. | [x] Rewrote `insertbyteimpl` per §6.1: pre-mutation `snapshotLines`; `bxscan` builds nframe (legacy `_draw` retained for tab Wid + nframe truncation); splice nframe.box into f.box; selection adjust; `f.relayoutFrom(0)`; `truncateOffscreen`; `diffLines(snap)`; `issuePaintOps`. Removed the legacy convergence loop, `drawtext` on staging frame, `fillNonGlyphAreas`, and `f.clean`. Updated `issuePaintOps` to handle Insert's downward blits (bottom-to-top order). Hardened `Init` and `Clear` to nil `f.lines` in lockstep with `f.box`. Added defensive bounds check in `lineDigest`. Stage-4: refined §3.5 diff to classify a line as dirty (paint) when its OLD or NEW position is not fully visible — off-screen Src pixels are stale, blitting them would copy old content into view. This was the load-bearing fix for the scroll-overlap bug. | [x] (pending) `frame: insertbyteimpl uses snapshot + diff (B2.3 R7)` |
| B2.3.R8 | [x] frame-layout-design §6.3 | [x] 4 new tests in `frame/set_style_range_diff_test.go` (R8.1–R8.4). All existing TestSetStyleRange_* green. `TestPaintParity_DrawtextAndRepaintAgreeOnFont` updated (Stage-4, with user approval) for the new real-style-transition pattern — R8 optimizes away no-op restyles, so parity is now tested via plain→bold instead of bold→bold. | [x] `SetStyleRange` now: `snapshotLines` → boundary `splitbox` at `[p0, p1)` edges → write Style+Wid on touched boxes → `relayoutFrom(0)` (eager-coalesce merges no-op boundary splits) → `diffLines(snap)` → `issuePaintOps`. The legacy `contentBottomY` shift-detection + conditional full-clear-vs-narrow-repaint branch is gone; the diff classifies styled lines as dirty (paint) and any height-changed line as dirty + shifts subsequent lines naturally. Dropped the `f.clean` call (R1's eager-coalesce subsumes it). Deleted the now-orphan `contentBottomY` helper. | [x] (pending) `frame: SetStyleRange uses diff for shift detection (B2.3 R8)` |
| B2.3.R9 | [x] frame-layout-design §6.4 — audit confirmed R6+R7 already cover this row's intent. | [x] Audit only. All Text-side scroll/fill paths (`Text.fill`, `Text.setorigin`, `Text.FrameScroll`, `Text.Redraw`) go through `fr.Insert` / `fr.InsertWithStyle` / `fr.Delete`, all of which now use snapshot+diff per R6+R7. No Text-side direct frame-pixel manipulation bypasses the diff machinery. `frame.Redraw` is just a bg fill before `t.fill` repaints content; the bg color is also what OpPaint clears with, so the two paths agree. | [x] No production code changes. The legacy "bulk-blit on the scroll path" referenced in the design was actually inside `bxscan`/`_draw` and `deleteimpl`/`insertbyteimpl`; R4/R6/R7 already removed those. Manual smoke test from R7 (open CLAUDE.md, scroll part-way down with B3 jumps) confirms no overlap / no stale glyphs / no double-paint. | [x] (pending) `frame: scroll/fill uses snapshot + diff path (B2.3 R9, audit-only)` |
| B2.3.R10 | [x] frame-layout-design §6.5 — audit confirmed the existing Text.Resize path works correctly via R1+R7. | [x] Audit only. `Text.Resize` → `t.fr.Clear(false)` (clears box + my R7 fix nils `f.lines`) → `t.Redraw` → `frame.Init` (resets state + new rect via `setrects`) → `frame.Redraw` (bg fill) → `t.fill` (chunked `fr.Insert` via R7). Each chunked Insert runs its own snapshot+diff. The R7 "off-screen blit fix" handles content that was off-screen pre-resize and is now visible (forces paint, not blit, so no stale-pixel import). | [x] No production code changes. The design's "single snapshot+diff over the entire resize" is a future optimization; the current "full clear + refill" path is correct and uses the diff machinery via t.fill. | [x] (pending) `text/frame: rect resize via snapshot + diff + Text refill (B2.3 R10, audit-only)` |
| B2.3.R11 | [x] frame-layout-design §5 (removed code paths) — scoped: see Notes. | [x] All frame tests green after the deletions. The R11 deletion set is the union of "orphan after R3/R6/R7/R8" walkers. | [x] **Deleted** (no remaining callers): `charofptimpl`, `ptofcharptb`, `ptofcharnb`, `grid` (was charofptimpl-only), `clean`, `chop`, `cklinewrap`, `advance`, the `points` struct, and `TestClean`. Switched `unlockedproxy.Charofpt` → `charOfPtReader`; switched `set_style_range_test.go` callers → `ptOfCharReader`. **Kept** (still live): `_draw` (used by bxscan for staging pt1 + nframe truncation), `cklinewrap0` (used by `_draw`), `lineHForAdvance` (used by `cklinewrap0`), `lineHAtPt` (used by `tick()`). Full deletion of the `_draw` family requires reworking bxscan's pt1 contract — a future row when needed (callers don't use pt1 in production; only TestBxscan tests it). | [x] (pending) `frame: delete orphan legacy walkers (B2.3 R11, partial)` |
| B2.3.R12 | [x] frame-layout-design §7 (I-LAYOUT-1..6) | [x] `go test ./frame/ -validateboxes` runs all 204 frame tests green with I-LAYOUT-2 / I-LAYOUT-3 / I-LAYOUT-4 / I-LAYOUT-6 hooked into `validateboxmodel`. The dedicated R1–R7 test files (`line_summary_test.go`, `lastlinefull_test.go`, `delete_diff_test.go`, `insert_diff_test.go`, `set_style_range_diff_test.go`) also assert the invariants directly. | [x] Extended `validateboxmodel` (frame/box.go) to assert I-LAYOUT-2 (line-table consistency), I-LAYOUT-3 (monotone TopY), I-LAYOUT-4 (lastlinefull formula), and I-LAYOUT-6 (no layout-only fragmentation). I-LAYOUT-1 (sole-writer for layout fields) is structurally guaranteed by the code shape — `relayoutFrom` is the only place writing X/Y/LineH/LineA — and there's no runtime way to check it without instrumenting every box-field assignment. I-LAYOUT-5 (paint matches layout) is paint-time and would need a `-validatelayout` hook in `paintBox`; deferred since validation across the test suite is already strong via the static invariants. | [x] (pending) `frame: enforce I-LAYOUT-2/3/4/6 under -validateboxes (B2.3 R12)` |
