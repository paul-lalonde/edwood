...
func (w *Window) applyParsedSpans(regionStart int, runs []spans.StyleRun, regions []*spans.Region, bufLen int) {
	...
	if len(regions) > 0 {
		...
		for _, r := range regions {
			if err := spans.TryAddRegion(w.regionStore, r); err != nil {
...
// buildStyledContent thinly wraps spans.Render with the
// Window's body and stores. Kept as a method for symmetry
// with the renderStyledFromBody callers.
func (w *Window) buildStyledContent() rich.Content {
	body := []rune(w.body.file.String())
	return spans.Render(body, w.spanStore, w.regionStore)
}
