...
| Tables (GFM, source-aligned columns) | ✓ | Phase 3 round 8 v1 — emits three nested region kinds: `table` around the block, `tablerow` per row (`header=true` on the first row), `tablecell` per cell with `align=left|right|center` parsed from the separator row's `:` markers. Detection requires a header row + a separator row; a `\|`-line without a separator stays plain text. Cell content uses `family=code` so the rendered cells are monospace — if your source has aligned columns (`\| H1   \| H2   \|`), they render aligned via character-position. Column-width-aware alignment (auto-padding ragged source) is deferred to round 8.x. Tables inside blockquotes work via the recursive parse path. |
