...
| Lists (single-line items, nested via leading whitespace) | ✓ | Phase 3 round 7 — emits `begin region listitem marker=X` (unordered, X is `-` / `*` / `+`) or `begin region listitem number=N` (ordered, N is the item number) per list line. Round 7 v1 covered column-0 only; round 7.x adds nesting via 2-spaces-or-tab-per-level leading whitespace (matches `markdown/parse.go`'s `tabCount + spaceCount/2` formula). Each list line is its own SIBLING region — depth lives in the source whitespace, not in the wire format. The marker must be followed by a SPACE (so `*foo*` stays emphasis and `-foo` stays plain text). Multi-line items via continuation lines remain deferred to round 7.y. Lists inside blockquotes work via the recursive parse path (the inside-blockquote line gets its layout indent shift via the round-7 smoke fix). |
